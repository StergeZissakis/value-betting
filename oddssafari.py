...
def find_max_bookie(page, max_odds, ul_xpath = '//div[@id="__next"]/div[2]/main/div[2]/div[4]/div[2]/div/div[2]/ul'):
    ret = []
...
def process_over_under_match(browser, page, match_id, event_date_time, click_element):
    ...
    print("Tabs: " + str(len(tab_buttons)))
    for tab_button in tab_buttons:
        ...
        for group in groups:
            ...
            browser.scroll_move_left_click(group, group_bookies_ul_xpath) 
...
def process_over_under_tab(browser, page, tab_button):
    ...
    if len(event_dates) != len(event_tables):
        print("Event Datesa and Tables mismatch: [" + str(len(event_dates)) + "] VS [" + str(len(event_tables)) + "]")
    for i in range(0, len(event_dates)):
        ...
        print("DateTime: " + str(event_date_time))
